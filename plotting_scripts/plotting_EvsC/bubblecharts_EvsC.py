...
# calculate pearson correlation coefficient between connectivity and entropy
pearson_corr, pearson_pvalue = pearsonr(connectivity, entropy)
print("Pearson correlation coefficient between connectivity and entropy:", pearson_corr)
print("Pearson correlation p-value between connectivity and entropy:", pearson_pvalue)

# calculate kendall tau correlation coefficient between connectivity and entropy
from scipy.stats import kendalltau
kendall_tau,  kendall_tau_pvalue = kendalltau(connectivity, entropy)
print("Kendall tau correlation coefficient between connectivity and entropy:", kendall_tau)
print("Kendall tau correlation p-value between connectivity and entropy:", kendall_tau_pvalue)
...
print("Standard deviation entropy:", std_entropy)
print("Median connectivity:", median_connectivity)
print("Median entropy:", median_entropy)

X = np.column_stack((connectivity, entropy)) # create a 2D array with connectivity and entropy
...
plt.scatter(connectivity[outlier_indices], entropy[outlier_indices], color=colors[outlier_indices], s=sizes[outlier_indices], hatch="++", alpha=0.4, edgecolors=colors[outlier_indices])
for i in outlier_indices:
    ...
    plt.annotate(city_names[i], 
                 (connectivity[i], entropy[i]), 
                 xytext=(connectivity[i] + xshift, entropy[i] + yshift),  # Offset
                 textcoords='data',
                 fontsize=9, 
                 color='grey',
...
plt.scatter(connectivity[regular_indices], entropy[regular_indices], color=colors[regular_indices], s=sizes[regular_indices], alpha=0.4,  edgecolors=colors[regular_indices])
#for i in regular_indices:
#   plt.text(connectivity[i] + np.log(sizes[i])/400, entropy[i], city_names[i], fontsize=8, ha='left', va='center')
...
#    plt.text(connectivity[i], entropy[i], country_names[i], fontsize=10, ha='right', va='bottom')
...
for i in highlight_cities_indices:
    ...
    if i in np.where(y_pred == -1)[0]:
        plt.scatter(connectivity[i], entropy[i], color=colors[i], s=sizes[i], alpha=0.8, edgecolors='k',hatch="++")
    else:
        plt.scatter(connectivity[i], entropy[i], color=colors[i], s=sizes[i], alpha=0.8, edgecolors='k')
...
# plot y = median_entropy
plt.axhline(median_entropy, color='grey', linestyle='--', alpha=0.5)
plt.text(0.1, median_entropy, r"Median $\mathcal{E}$: "+f"{median_entropy:.2f}", fontsize=9, ha='left', va='bottom')
...
plt.text(0.1, 0.1, r"$\mathcal{C} = \mathcal{E}$", fontsize=9, ha='left', va='bottom')

# Labels and Titles
plt.xlabel("Normalized closeness")
plt.ylabel("PoI-entropy")
...
# Show plot
transparent = False
plt.savefig("plotting_EvsC/plots/EvsC_allcities", transparent=transparent, dpi=600)
plt.show()
