...
import utils
data = json.load(open("json/ranking_proximity.json"))

sorted_cities = sorted(data.items(), key=lambda x: x[1]["mean"])
...
for mean in boxplot['means']:
    ...
    mean.set_linestyle(' ')
    mean.set_linewidth(0)

for i, patch in enumerate(boxplot['boxes']):
    patch.set_facecolor(
        utils.continent_to_color[
            utils.city_to_continent[sorted_city_names[i]]
        ]
    )
...
yticks = [0, 5, 10, 15, 20, 30, 60]
plt.yticks(yticks)
plt.ylim(0, max(yticks) + 5)
