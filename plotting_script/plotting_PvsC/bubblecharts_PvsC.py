...
# calculate pearson correlation coefficient between connectivity and proximity
pearson_corr, pearson_pvalue = pearsonr(connectivity, proximity)
print("Pearson correlation coefficient between connectivity and proximity:", pearson_corr)
print("Pearson correlation p-value between connectivity and proximity:", pearson_pvalue)

# calculate kendall tau correlation coefficient between connectivity and proximity
from scipy.stats import kendalltau
kendall_tau,  kendall_tau_pvalue = kendalltau(connectivity, proximity)
print("Kendall tau correlation coefficient between connectivity and proximity:", kendall_tau)
print("Kendall tau correlation p-value between connectivity and proximity:", kendall_tau_pvalue)
...
print("Mean proximity:", mean_proximity)
print("Standard deviation proximity:", std_proximity)
print("Median connectivity:", median_connectivity)
print("Median proximity:", median_proximity)

X = np.column_stack((connectivity, proximity)) # create a 2D array with connectivity and proximity
...
plt.scatter(connectivity[outlier_indices], proximity[outlier_indices], color=colors[outlier_indices], s=sizes[outlier_indices], hatch="++", alpha=0.4, edgecolors=colors[outlier_indices])
for i in outlier_indices:
    ...
    plt.annotate(city_names[i], 
                 (connectivity[i], proximity[i]), 
                 xytext=(connectivity[i] + xshift, proximity[i] + yshift),  # Offset
                 textcoords='data',
                 fontsize=9, 
                 color='grey',
...
plt.scatter(connectivity[regular_indices], proximity[regular_indices], color=colors[regular_indices], s=sizes[regular_indices], alpha=0.4,  edgecolors=colors[regular_indices])
#for i in regular_indices:
#   plt.text(connectivity[i] + np.log(sizes[i])/400, proximity[i], city_names[i], fontsize=8, ha='left', va='center')
...
#    plt.text(connectivity[i], proximity[i], country_names[i], fontsize=10, ha='right', va='bottom')
...
for i in highlight_cities_indices:
    ...
    if i in np.where(y_pred == -1)[0]:
        plt.scatter(connectivity[i], proximity[i], color=colors[i], s=sizes[i], alpha=0.8, edgecolors='k',hatch="++")
    else:
        plt.scatter(connectivity[i], proximity[i], color=colors[i], s=sizes[i], alpha=0.8, edgecolors='k')
...
# plot y = median_proximity
plt.axhline(median_proximity, color='grey', linestyle='--', alpha=0.5)
plt.text(0.1, median_proximity, r"Median $\mathcal{P}$: "+f"{median_proximity:.2f}", fontsize=9, ha='left', va='bottom')
...
plt.text(0.1, 0.1, r"$\mathcal{C} = \mathcal{P}$", fontsize=9, ha='left', va='bottom')

# Labels and Titles
plt.xlabel("Normalized closeness")
plt.ylabel("PoI-proximity")
...
# Show plot
transparent = False
plt.savefig("plotting_PvsC/plots/PvsC_allcities", transparent=transparent, dpi=600)
plt.show()
