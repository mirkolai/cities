...
# calculate pearson correlation coefficient between connectivity and density
pearson_corr, pearson_pvalue = pearsonr(connectivity, density)
print("Pearson correlation coefficient between connectivity and density:", pearson_corr)
print("Pearson correlation p-value between connectivity and density:", pearson_pvalue)

# calculate kendall tau correlation coefficient between connectivity and density
from scipy.stats import kendalltau
kendall_tau,  kendall_tau_pvalue = kendalltau(connectivity, density)
print("Kendall tau correlation coefficient between connectivity and density:", kendall_tau)
print("Kendall tau correlation p-value between connectivity and density:", kendall_tau_pvalue)
...
X = np.column_stack((connectivity, density)) # create a 2D array with connectivity and density
...
# avoid plotting points with zero connectivity
valid_indices = np.where(connectivity > 0.0)[0]

# keep only outlier indices that are also valid indices
outlier_indices = np.intersect1d(outlier_indices, valid_indices)
# remove highlight cities from outlier indices
outlier_indices = np.setdiff1d(outlier_indices, highlight_cities_indices)
plt.scatter(connectivity[outlier_indices], density[outlier_indices], color=colors[outlier_indices], s=sizes[outlier_indices], hatch="++", alpha=0.4, edgecolors=colors[outlier_indices])
for i in outlier_indices:
    ...
    plt.annotate(city_names[i], 
                 (connectivity[i], density[i]), 
                 xytext=(connectivity[i] + xshift, density[i] + yshift),  # Offset
                 textcoords='data',
                 fontsize=9, 
                 color='grey',
                 bbox=dict(boxstyle="round,pad=0.3", edgecolor='grey', facecolor=colors[i], alpha=0.2), 
                 arrowprops=dict(arrowstyle="->", color='black', lw=0.8))
# Plot all points in blue
# regular points are cities that are not outliers and for which connectivity is greater than 0
regular_indices = np.where((y_pred == 1))
# avoid plotting points with zero connectivity
valid_indices = np.where(connectivity > 0.0)[0]

# keep only regular indices that are also valid indices
regular_indices = np.intersect1d(regular_indices, valid_indices)
# remove highlight cities from regular indices
regular_indices = np.setdiff1d(regular_indices, highlight_cities_indices)

plt.scatter(connectivity[regular_indices], density[regular_indices], color=colors[regular_indices], s=sizes[regular_indices], alpha=0.4,  edgecolors=colors[regular_indices])
#for i in regular_indices:
#   plt.text(connectivity[i] + np.log(sizes[i])/400, density[i], city_names[i], fontsize=8, ha='left', va='center')
...
#    plt.text(connectivity[i], density[i], country_names[i], fontsize=10, ha='right', va='bottom')
...
for i in highlight_cities_indices:
    ...
    if i in np.where(y_pred == -1)[0]:
        plt.scatter(connectivity[i], density[i], color=colors[i], s=sizes[i], alpha=0.8, edgecolors='k',hatch="++")
    else:
        plt.scatter(connectivity[i], density[i], color=colors[i], s=sizes[i], alpha=0.8, edgecolors='k')
...
# plot y = median_density
plt.axhline(median_density, color='grey', linestyle='--', alpha=0.5)
plt.text(0.1, median_density, r"Median $\mathcal{D}$: "+f"{median_density:.2f}", fontsize=9, ha='left', va='bottom')
...
plt.text(0.1, 0.1, r"$\mathcal{C} = \mathcal{D}$", fontsize=9, ha='left', va='bottom')
...
legend1 = plt.legend(title="Population", scatterpoints=1, labelspacing=0.5, loc='upper right', bbox_to_anchor=(1.02,1.03), handletextpad=1.5, frameon=False)
...
legend2 = plt.legend(handles=handles[len(handles)-len(continent_to_color):], scatterpoints=1, labelspacing=0.5, loc='upper left', handletextpad=1, frameon=False)
...
# Show plot
transparent = False
plt.savefig("plotting_DvsC/plots/DvsC_allcities", transparent=transparent, dpi=600)
plt.show()
